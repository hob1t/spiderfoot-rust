...
const BLOCKLISTDE_URL: &str = "https://lists.blocklist.de/lists/all.txt";
...
impl SfpBlocklistde {
    ...
    async fn get_blacklist(
        ...
        emitter.log(LogLevel::Info, "Fetching blocklist.de blacklist...");
        let url = options
            .custom
            .get("_test_url")
            .map(|s| s.as_str())
            .unwrap_or(BLOCKLISTDE_URL);
        ...
        let res = match self.client.get(url).send().await {
            ...
            Err(e) => {
                emitter.log(
                    LogLevel::Error,
                    &format!("Failed to fetch blocklist.de: {}", e),
                );
        ...
        if !res.status().is_success() {
            emitter.log(
                LogLevel::Error,
                &format!(
                    "Unexpected HTTP response from blocklist.de: {}",
                    res.status()
                ),
            );
            self.error_state.store(true, Ordering::Relaxed);
        ...
        let body = match res.text().await {
            ...
            Err(e) => {
                emitter.log(
                    LogLevel::Error,
                    &format!("Failed to read blocklist.de body: {}", e),
                );
                self.error_state.store(true, Ordering::Relaxed);
                return None;
            }
    ...
    async fn execute_inner(
        ...
        let (malicious_type, blacklist_type, is_netblock) = match kind {
            "IP-ADDR" => ("MALICIOUS_IPADDR", "BLACKLISTED_IPADDR", false),
            "NETBLOCK_WHOIS" => {
        ...
        if found {
            let url = format!("https://www.blocklist.de/en/search.html?ip={}", event_data);
            let text = format!("blocklist.de [{}]\n{}", event_data, url);
...
impl Default for SfpBlocklistde {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl SpiderfootModule for SfpBlocklistde {
    fn name(&self) -> &'static str {
        "sfp_blocklistde"
    }

    fn description(&self) -> &'static str {
        "Check if an IP address or netblock is malicious according to blocklist.de."
    }

    fn target_types(&self) -> &'static [&'static str] {
        &["IP-ADDR", "NETBLOCK_WHOIS"]
    }
...
mod tests {
    ...
    #[test]
    fn test_parse_blacklist_comments_and_empty() {
        let module = SfpBlocklistde::new();
