//! integration tests for `sfp_cleantalk`

#[path = "common/mod.rs"]
mod common;

use spiderfoot_rust::core::{ModuleOptions, SpiderfootModule, Target};
use spiderfoot_rust::modules::sfp_cleantalk::SfpCleantalk;
...
async fn test_module_metadata() {
    ...
    assert_eq!(module.name(), "sfp_cleantalk");
    assert!(module.description().contains("CleanTalk.org"));
...
async fn test_module_execution_listed_ip() -> Result<(), Box<dyn Error + Send + Sync>> {
    ...
    let mock_url = format!("{}/ipset", server.uri());
